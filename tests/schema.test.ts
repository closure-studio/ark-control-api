  ...
  it("creates the current tables, columns, and indexes", () => {
    ...
    try {
      ...
      expect(columns("arknights_maintenance_announcements")).toEqual([
        ...
        "pre_action_at",
        "pre_action_state",
        "pre_action_started_at",
        "pre_action_completed_at",
        "pre_action_failed_step",
        "pre_action_error_message",
        "notified",
        "error_message"
      ]);
      expect(columns("arknights_maintenance_host_runs")).toEqual([
        "id",
        "announcement_news_id",
        "host_id",
        "host_name_snapshot",
      ...
      expect(indexes).toEqual(
        expect.arrayContaining([
          ...
          "idx_maintenance_pre_action_due",
          "idx_maintenance_host_runs_announcement"
        ])
      );
      expect(indexes).not.toEqual(
        expect.arrayContaining([
          ...
          "idx_maintenance_pre_action_claim_expiry"
        ])
      );
      ...
      expect(queryPlan).toEqual([expect.stringContaining("USING INDEX idx_vps_hosts_arkhost")]);
    } finally {
      db.close();
    }
  });
    ...
    try {
      ...
      db.exec(`
        ...
        INSERT INTO arknights_maintenance_announcements (
          news_id, url, processing_state, first_seen_at, pre_action_state
        ) VALUES (
          '1459', 'https://ak.hypergryph.com/news/1459', 'completed',
          '2026-01-01T00:00:00.000Z', 'completed'
        );
        INSERT INTO arknights_maintenance_host_runs (
          id, announcement_news_id, host_id, host_name_snapshot,
          host_address_snapshot, status, created_at, updated_at
        ) VALUES (
          1, '1459', 1, 'Host', '192.0.2.1', 'succeeded',
          '2026-01-01T00:00:00.000Z', '2026-01-01T00:00:00.000Z'
        );
      ...
      expect(
        db
          .prepare(
            "SELECT host_id, host_name_snapshot FROM arknights_maintenance_host_runs WHERE id = 1"
          )
          .get()
      ).toMatchObject({ host_id: null, host_name_snapshot: "Host" });

      db.exec("DELETE FROM arknights_apk_releases WHERE id = 1");
      expect(
        db.prepare("SELECT id FROM arknights_apk_host_runs WHERE id = 1").get()
      ).toBeUndefined();
      db.exec("DELETE FROM arknights_maintenance_announcements WHERE news_id = '1459'");
      expect(
        db.prepare("SELECT id FROM arknights_maintenance_host_runs WHERE id = 1").get()
      ).toBeUndefined();
    } finally {
      db.close();
    }
