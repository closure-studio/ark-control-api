...
**Deployment**:
The rollout of one APK Release to every Redroid VPS Host that is enabled when the rollout begins. It contains one Host Run per targeted VPS Host.
_Avoid_: Pipeline, batch

**VPS Host Role**:
The fixed workload responsibility of a VPS Host, either Redroid or Arkhost.
_Avoid_: Type, kind
